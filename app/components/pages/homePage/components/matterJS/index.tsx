...
export function MatterJSScene() {
  ...
  useEffect(() => {
    ...
    matterRef.current.addEventListener("click", (event) => {
      ...
      const rotatedX =
        relativeX * Math.cos(-bannerAngle) - relativeY * Math.sin(-bannerAngle);
      const rotatedY =
        relativeX * Math.sin(-bannerAngle) + relativeY * Math.cos(-bannerAngle);
  ...
  return (
    <div className="relative w-screen h-screen">
      <div
        className={
          "left-0 top-0 w-full h-full absolute " + style.matterContainer
        }
        ref={matterRef}
      ></div>
    </div>
